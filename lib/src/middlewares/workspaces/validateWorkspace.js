...
const validateWorkspace = async (app, firestore, req, res, next) => {
    ...
    try {
        if (!workspaceId)
            throw new Error(JSON.stringify(errors_msg_1.WORKSPACE_IS_REQUIRED));
        if (!userToken)
            throw new Error(JSON.stringify(errors_msg_1.USER_TOKEN_IS_REQUIRED));
        ...
        if (!workspaceList.length)
            throw new Error(JSON.stringify(errors_msg_1.WORKSPACE_NOT_FOUND));
        ...
        if (!cachedWorkspaceData || (new Date(cachedWorkspaceData?.timestamp)?.getTime() + operations_msg_1.FIVE_MIN) < currentTime.getTime()) {
            ...
            if (!active)
                throw new Error(JSON.stringify(errors_msg_1.WORKSPACE_NOT_ACTIVE));
